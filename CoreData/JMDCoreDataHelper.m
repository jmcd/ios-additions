...
- (id)init {
	...
	if (self) {
		...
		NSAssert(URL, @"error constructing url for filename %@", modelFilename);

		NSManagedObjectModel *model = [[NSManagedObjectModel alloc] initWithContentsOfURL:URL];
		NSAssert(model, @"error init'ing model with url %@", URL.absoluteString);

		NSPersistentStoreCoordinator *coordinator = [[NSPersistentStoreCoordinator alloc] initWithManagedObjectModel:model];
		NSAssert(coordinator, @"error init'ing persistent-store-coordinator with model %@", model);
		...
		NSAssert(store, @"error adding new persistent store of type %@ at url %@", NSSQLiteStoreType, storeURL.absoluteString);
...
- (NSArray *)executeFetchRequestOnMainQueueContext:(NSFetchRequest *)request {
	...
	NSAssert(result, @"error executing fetch request: %@, %@", error.localizedDescription, error.userInfo);
	return result;
}
...
- (NSUInteger)countForFetchRequestOnMainQueueContext:(NSFetchRequest *)request {
	...
	NSAssert(error == nil, @"error executing fetch request: %@, %@", error.localizedDescription, error.userInfo);
	return result;
}
...
- (void)saveContext:(NSManagedObjectContext *)context {
	...
	NSError *error = nil;
	NSAssert([context save:&error], @"error saving context: %@, %@", [error localizedDescription], [error userInfo]);
}
...
- (NSManagedObject *)existingObjectWithID:(NSManagedObjectID *)objectID inContext:(NSManagedObjectContext *)context {
	...
	NSAssert(object, @"error fetching existing object: %@, %@", error.localizedDescription, error.userInfo);
	return object;
}
